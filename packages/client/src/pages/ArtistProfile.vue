  ...
  <div class="flex size-full flex-col items-center gap-28 overflow-auto bg-gradient-to-t from-pink-100 px-10 py-14 md:px-40">
    <div class="flex max-w-[50rem] flex-col gap-2 rounded bg-white/90 px-5 py-3 shadow-sm">
      <div class="flex w-full flex-col items-center gap-2 sm:flex-row">
        <img class="h-32 w-40 rounded bg-cover object-cover" :src="loadImage(artist?.picture)" alt="artistPfp" />
        <div class="flex w-full flex-col justify-between sm:flex-row">
          <div class="font-medium">
            <div class="flex flex-row items-center gap-2">
              <Icon icon="person" class="font-light" />
              <p>
                {{ artist?.name }}
              </p>
            </div>
            <div class="flex flex-row items-center gap-2">
              <Icon icon="mail" class="font-light" />
              <p>
                {{ artist?.email }}
              </p>
            </div>
          </div>
          <button v-i18n class="btn-outlined sm:text-md mt-5 text-sm" @click="router.push({ name: 'book' })">Book me</button>
        </div>
      </div>
    ...
    <div class="flex flex-col items-center justify-center gap-5">
      <div class="flex flex-row items-center justify-center gap-4">
        <p class="flex-start font-fancy text-5xl">Gallery</p>
        <Icon
      ...
      <div v-if="gallery?.length >= 1 && !edit" class="flex w-full flex-row flex-wrap items-center justify-center gap-5">
        <img
      ...
      <p v-else-if="!edit" class="text-center">This artist doesn't have any images to show</p>
    </div>
  </div>
</template>

...
const isSelected = (option) => modelValue.value.some((o) => o.value === option.value);
...
const removeOption = (option) => {
  modelValue.value = modelValue.value.filter((o) => o.value !== option.value);
};
</script>
  ...
  <div ref="dropdown" class="relative w-full">
    <div class="flex cursor-pointer flex-wrap gap-2 rounded border border-gray-300 p-2" @click="toggleDropdown">
      <span
        ...
        {{ option.label }}
        <button class="text-pink-500 hover:text-pink-800" @click.stop="removeOption(option)">&times;</button>
      </span>
